          ...
          {[displayLeaderboard[1], displayLeaderboard[0], displayLeaderboard[2]].map((entry, idx) => {
            ...
            return (
              <div key={`podium-${entry.rank}`} className={`flex flex-col items-center ${entry.isCurrentUser ? 'relative' : ''}`}>
                {entry.isCurrentUser && (
                  <span className="absolute -top-2 px-2 py-0.5 rounded-full bg-purple-500 text-white text-[10px] font-bold z-10">
                    {t('leaderboard.you')}
                  </span>
                )}
                <div className="text-4xl mb-2">{entry.avatar}</div>
                <p className={`font-bold text-sm mb-1 truncate max-w-full ${entry.isCurrentUser ? 'text-purple-600 dark:text-purple-400' : 'text-slate-900 dark:text-white'}`}>{entry.name}</p>
                <p className="text-amber-600 dark:text-amber-400 font-semibold text-xs mb-2">{entry.xp.toLocaleString()} XP</p>
                <div className={`w-full ${heights[idx]} rounded-t-xl bg-gradient-to-t ${entry.isCurrentUser ? 'from-purple-400 to-purple-500 dark:from-purple-500 dark:to-purple-600' : colors[idx]} flex items-end justify-center pb-3`}>
          ...
          {displayLeaderboard.map((entry) => (
            <div
              ...
              className={`grid grid-cols-[60px_1fr_100px_80px] sm:grid-cols-[60px_1fr_120px_100px_100px] gap-2 p-4 items-center border-b border-slate-100 dark:border-gray-800/50 hover:bg-slate-50 dark:hover:bg-gray-800/30 transition-colors ${
                ...
                entry.isCurrentUser ? 'ring-2 ring-purple-500/50 bg-purple-50/50 dark:bg-purple-500/10 rounded-lg' : ''
              }`}
            >
              ...
              <div className="flex items-center gap-3">
                ...
                <span className={`font-medium text-sm truncate ${entry.isCurrentUser ? 'text-purple-700 dark:text-purple-300' : 'text-slate-900 dark:text-white'}`}>{entry.name}</span>
                {entry.isCurrentUser && (
                  <span className="px-1.5 py-0.5 rounded-full bg-purple-100 dark:bg-purple-500/20 text-purple-700 dark:text-purple-300 text-[10px] font-bold uppercase">
                    {t('leaderboard.you')}
                  </span>
                )}
              </div>
