...
// SolForge Academy collection/creator addresses for filtering
// Replace with actual addresses once cNFT minting is deployed
const SOLFORGE_CREATORS: string[] = [];
...
export async function GET(
  ...
  try {
    ...
    // Filter to only SolForge Academy certificates
    // When SOLFORGE_CREATORS is empty, return all NFTs (useful during dev)
    const filtered =
      SOLFORGE_CREATORS.length > 0
        ? assets.filter(
            (asset) =>
              asset.authorities?.some((auth) =>
                SOLFORGE_CREATORS.includes(auth.address)
              ) ||
              asset.grouping?.some(
                (group) =>
                  group.group_key === "collection" &&
                  SOLFORGE_CREATORS.includes(group.group_value)
              )
          )
        : assets;
    ...
    const certificates = filtered.map((asset) => ({
      ...
      name: asset.content?.metadata?.name || "SolForge Certificate",
      symbol: asset.content?.metadata?.symbol || "SFC",
